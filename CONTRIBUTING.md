...
- Official FIRST STEP/CAD files, raw logs, checkpoints, SQLite DBs, or `.env` files (`var/` stays gitignored)
...
- [ ] No secrets, venv, `node_modules`, or `var/` contents
...
2. Confirm `git ls-files` has no `.env`, `*.db`, `*.log`, `.venv`, STEP/CAD, or checkpoint zips
