...
def get_evaluation(eid: str) -> dict[str, Any] | None:
    ...
    if not row:
        return None
    return {"id": row["id"], "runId": row["run_id"], "report": json.loads(row["report"])}


def list_evaluations() -> list[dict[str, Any]]:
    rows = connect().execute("SELECT id, run_id, report FROM evaluations ORDER BY created_at DESC").fetchall()
    return [{"id": r["id"], "runId": r["run_id"], "report": json.loads(r["report"])} for r in rows]
