  ...
  return (
    <main className="page layout-replay">
      <Panel
        ...
        sub={active ? <span className="mono">{active}</span> : undefined}
        actions={
          <>
            <Segmented
              label="Camera"
            ...
            <button type="button" className="btn sm" disabled={!active} onClick={() => setExportId(active)}>
      ...
      <div className="col">
        ...
        >
          ...
          <ul className="list">
            {(ids || []).map((r) => (
              <li key={r.id}>
                <button type="button" className={`list-item ${r.id === active ? "on" : ""}`} onClick={() => selectReplay(r.id)}>
                  <span className="grow">
