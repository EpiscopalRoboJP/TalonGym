...
Catalog assembly editor: a guided Start → Assemble → Configure → Check & test → Use in training workflow, with a mount-first 3D workspace, compatibility-filtered goBILDA/REV catalog, persistent assembly tree, and contextual inspector. Shipped starters open read-only until **Edit a copy** creates a working draft. Select a part and then any visible mount node; the drawer shows only compatible parts and creates an exact preview before confirmation. Connected children move through their mate definitions. Detached subassemblies remain visible and editable as loose roots, but must be attached before publishing. Numeric positioning is limited to component roots because connected child poses are solved by their mounts. Replacement is allowed only when every existing mount ID, hole index, and hardware connection remains compatible. The wizard instantiates real rails, brackets, motors, and wheels as a schema 1.2 assembly. Inference proposes drivetrain, transmissions, and part roles; **Confirm inference** is required before a competitive save can enable `physical_actuators`. Unconfirmed robots stay on `high_level_waypoint`. Scoring topology is compiled only when the assembly declares it (`includeScoringTopology` or intake/flywheel parts); drivebases no longer inherit a hidden 4-cap mechanism.
...
Official manufacturer CAD downloads on demand into `var/assets/robot_parts/` (not git). Archive imports require an exact configured member/SKU match or a single unambiguous model; the importer rejects multi-model archives instead of guessing by file size. Conversion normalizes units, axes, and origin once, and the browser renders that verified result without a second proxy-derived rescale. Each successful conversion also generates a deterministic isometric thumbnail. Cache is `ready` only when the GLB and thumbnail both exist and load. Committed viewport parts render that GLB (collision proxy only while loading or after a load error). **Cache required CAD** lists missing SKUs on the current assembly, queues them, and refreshes those parts when the batch finishes. **Cache all CAD** queues every downloadable uncached SKU through bounded background jobs (`POST /api/v1/catalog/cache/all`) with progress, cancel, retry-failed, and thumbnail generation; it never blocks the catalog UI. Without `[cad]`, or when a SKU has download disabled, each part remains clearly labeled **Proxy** with its authored collision geometry — never a single chassis lump when catalog parts exist. Per-part GLB upload remains `POST /api/v1/presets/robot/{id}/parts/{partId}/model`.

**Check simulation model** compiles the exact draft on the Python backend and compares every returned instance pose against the browser solver before the design can be trusted for training. The compiler reports proxy overlaps and identifies the drivetrain model as a calibrated chassis-force approximation. Catalog wheel geometry does not create per-wheel contact dynamics by itself.
